...
export default function StoriesSection({ stories, handleProfileClick }) {
  ...
  return (
    <section id="stories" className="mx-auto max-w-4xl px-6 py-12">
      ...
      {stories.length === 0 ? (
        <div className={`mt-6 rounded-3xl border border-dashed ${activeTheme.inputBorder} ${activeTheme.sectionBg} backdrop-blur p-8 ${activeTheme.mutedText}`}>
        ...
        <div className="mt-6 flex flex-col items-center gap-4">
          <div className={`w-full max-w-md h-[280px] flex flex-col rounded-2xl border ${activeTheme.cardBorder} ${activeTheme.cardBg} p-6 backdrop-blur`}>
            <div>
              ...
              <h3 className={`text-xl font-medium ${activeTheme.primaryText} mt-1`}>
                {stories[currentStoryIndex].title}
              </h3>
            </div>

            <div className={`mt-3 ${activeTheme.mutedText} leading-7 overflow-y-auto`}>
